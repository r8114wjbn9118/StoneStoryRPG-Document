...
`loc.begin`		仅在开始进入任意地区时，`time`为`0`的时候，第一帧的结果为`true`。不包括由衔尾蛇之石自动开始的循环游戏。可以用来初始化变量
...
`totaltime`		在当前大地区的帧号，进入boss房也会累计（进入boss房间时，`time`会重置为`0`，`totaltime`不重置；当重新进入关卡或者循环时，`time`和`totaltime`都会重置为0）
...
`summon.GetTime(index = 0)`		返回召唤物在当前状态下经过的帧数。`index`参数是可选的，默认为`0`(第一个召唤物)；如果该索引处没有召唤物，则返回`null`。
...
`import (script)`		加载并执行外部脚本的单一副本（即对一个外部脚本的所有的import 都是去加载同一份副本），详见[11.导入外部脚本](#·-·-·-11-导入外部脚本-importing-external-scripts-·-·-·)
...
`color.Lerp(c1,c2,t)`		*返回`string`*		计算从颜色`c1`到`c2`的在比例`t`(百分比)处的线性插值
...
`math.e`		*返回`float`*		常数`e`，也称为欧拉数或自然常数，约为2.71828
...
`storage.Set(string, value)`		*无返回值*		将`value`值保存到指定关键词的永久存储区
...
`string.IndexOf(str, criteria, startAt)`		*返回`int`*		接受字符串变量、字符串条件和起始索引作为参数。从`startAt`索引处开始搜索字符串，查找条件的位置；如果未找到，则返回-1
...
`te.GetTID(str)`		*返回`string`*		返回给定文本的文本标识符(`TID`)；输入文本应该是玩家选择的语言
...
`a.Contains(value)`		确定给定值是否在数组内；如果能找到则返回`true`，否则为`false`
...
`component.ax`		*返回`string`*		锚点的X部分，可能的值:`left`,`center`,`right`
...
`text.align`		*返回`string`*		框内文本的对齐方式。默认值为`左`。可能的值:`left`,`center`,`right`

`text.color`		*返回`string`*		文本的颜色，详情见[RGB转十六进制颜色表示法](https://htmlcolorcodes.com/)。

`text.lines`		*返回`string[]`*		字符串数组，内容为文本框格式化其内容后分解的文本行。不包括颜色元数据。

text.text		*返回`string`*		文本框的全部内容。文本的子部分可以用元数据`[color=#rrggbb][/color]` 来着色。
...
`button.text`		*返回`string`*		写在按钮内部的文本
...
`button.tcolor`		*返回`string`*		按钮内文本的颜色，颜色详情见[RGB转十六进制颜色表示法](https://htmlcolorcodes.com/)。
...
`button.bcolor`		*返回`string`*		按钮边框的颜色，颜色详情见[RGB转十六进制颜色表示法](https://htmlcolorcodes.com/)。
...
`button.hcolor`		*返回`string`*		按钮被按下时高亮显示的颜色，颜色详情见[RGB转十六进制颜色表示法](https://htmlcolorcodes.com/)。
...
`button.sound`		*返回`string`*		按下按钮时播放的[声音效果](#·-·-·-附录b-音乐效果-sound-effects-·-·-·)。默认为“确认”。
...
`button.style`		*返回`int`*		按钮当前样式的ID号，默认值为1；可能的值从-8到8。可以通过`ui.AddStyle`()添加其他样式。
...
`anim.color`		*返回`string`*		动画的颜色，颜色详情见[RGB转十六进制颜色表示法](https://htmlcolorcodes.com/)。

`anim.duration`		*返回`int`*		动画的时间长度，以帧为单位。

`anim.flipX`		*返回`bool`*		如果为`true`，则以其轴心水平翻转图片。

`anim.flipY`		返回bool`*		如果为`true`，则以其轴心竖直翻转图片。

`anim.frame`		*返回`int`*		正在绘制的当前动画帧位置，可以通过更改以将动画设置到特定帧。

`anim.gamePause`		*返回`bool`*		如果为`true`，当玩家暂停游戏时动画会自动暂停播放，而当玩家退出暂停状态时，动画会继续播放。

`anim.loop`		*返回`bool`*		如果为`true`，动画将在持续时间结束时从头开始播放。

`anim.playing`		*返回`bool`,只读*		如果动画当前正在播放，则为`true`。

`anim.paused`		*返回`bool`,只读*		如果动画当前正在播放，则为`true`，但由于调用`anim.Pause()`而暂停

`anim.pivotX`/`anim.pivotY`		*返回`int`*		可用于微调ASCII艺术相对于其位置绘制位置的中心附加偏移量。

`anim.playOnStart`		*返回`bool`*		如果为真，动画将尽快开始播放。
...
`canvas.blend`		*返回`string`*		画布的混合模式，当画布后面有元素时。可能的值:`opaque`不透明、`Multiply`乘、`Divide`除、`Add`加、`Subtract`减。默认值为`opaque`不透明。
...
`canvas.Get(x,y)`		*返回`string`*		返回画布上特定位置`x`，`y`处的符号。
...
`canvas.Set(x,y,str)`		*无返回值*		将画布上的特定位置`x`，`y`更改为`str`。
...
`canvas.SetFG(x,y, color)`		*无返回值*		更改特定位置`x`，`y`的前景色。
...
`canvas.SetBG(x,y, color)`		*无返回值*		更改特定位置`x`，`y`的背景色。
...
部分相关内容请参照[5.搜索过滤器/关键词 Search Filters](#·-·-·-5-搜索过滤器-关键词-search-filters-·-·-·)
...
感谢up主 [福阿月](https://space.bilibili.com/817710) 所进行的[翻译工作](https://www.bilibili.com/read/cv6901881)，本文档在他的基础上完成（节省了大量复制粘贴的时间）。
